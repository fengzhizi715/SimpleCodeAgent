...
def build_default_registry(*, enable_reviewer: bool = True, enable_external_coder: bool = True) -> AgentRegistry:
    """构造默认 Agent Registry。
    ...
    if enable_external_coder:
        registry.register(ExternalCodingAgent())
    if enable_reviewer:
...
def build_orchestrator_runtime(
    ...
    *,
    ...
    enable_external_coder: bool = True,
) -> OrchestratorRuntime:
    """构造默认 V2 Orchestrator Runtime。"""
    return OrchestratorRuntime(
        registry=build_default_registry(
            enable_reviewer=enable_reviewer,
            enable_external_coder=enable_external_coder,
        ),
