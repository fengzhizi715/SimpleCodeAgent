<template>
  <section class="panel">
    <h2>RAG 文档检索</h2>
    <p class="muted">
      展示当前向量库（Chroma）中的文件与 chunk 分布，以及运行时使用的 Embedding 模型配置。
    </p>
  ...
  <section class="panel" v-if="!overview && !loading && !error">
    <p class="muted">暂无 RAG 数据。</p>
  </section>
</template>

<script setup>
import { computed, onMounted, ref } from "vue";
import { deleteRagSource, getRagOverview, reindexRagSource, uploadRagFile } from "../api";
...
const topFiles = computed(() => {
  const rows = Array.isArray(overview.value?.files) ? overview.value.files : [];
  return rows;
});
...
async function submitUpload() {
  ...
  error.value = "";
  try {
    const result = await uploadRagFile(selectedFile.value, uploadDir.value || "uploads");
    uploadMessage.value = `已导入 ${result.source}，新增 chunks: ${result.ingested_chunks}`;
...
async function removeSource(item) {
  ...
  try {
    ...
    uploadMessage.value = `已删除 ${result.source}，删除 chunks: ${result.deleted_chunks}`;
    await loadOverview();
  } catch (err) {
...
async function reindexSource(item) {
  ...
  uploadMessage.value = "";
  try {
    const result = await reindexRagSource(source);
    uploadMessage.value = `已重建 ${result.source}，删除 chunks: ${result.deleted_chunks}，新建 chunks: ${result.ingested_chunks}`;
    await loadOverview();
  } catch (err) {
